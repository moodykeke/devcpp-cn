...
begin
  ...
  //fFile := devDirs.Config + DEV_CODEINS_FILE;
  if not FileExists(fFile) then begin
    AddItemByValues('$COMMENTHEADER$', 'Comment Header',
    '/*********************************************************************' + #13#10 +
    '    $PROGRAM$: ' + #13#10 +
    '    $COPYRIGHT$: ' + #13#10 +
    '    $AUTHOR$: ' + #13#10 +
    ...
    AddItemByValues('C main', 'C main', '#include <stdio.h>' + #13#10 + #13#10 +
      'int main() {' + #13#10 +  '	*|*' + #13#10 + #13#10 +
      '	return 0;' + #13#10 + '}' + #13#10, 0);
    AddItemByValues('C++ main', 'C++ main', '#include <iostream>' + #13#10 +
      'using namespace std;' + #13#10 + #13#10 +
      'int main() {' + #13#10 +  '	*|*' + #13#10 + #13#10 +
      '	return 0;' + #13#10 + '}' + #13#10, 0);

    AddItemByValues('$DATETIME$', 'Date Time', '<DATETIME> *|*', 1);
    AddItemByValues('for ()', 'for loop', 'for (*|*; ; ) {' + #13#10 + '}', 1);
    AddItemByValues('while ()', 'while loop', 'while (*|*) {' + #13#10 + '}', 1);
    AddItemByValues('do-while ()', 'do-while loop', 'do {' + #13#10 + '} while (*|*);', 1);
    AddItemByValues('if ()', 'if statement', 'if (*|*) {' + #13#10 + '}', 1);
    AddItemByValues('switch ()', 'switch statement', 'switch (*|*) {' + #13#10 + '	default:' + #13#10 + '}', 1);
  ...
  //Default Code
  if not FileExists(devDirs.Config + DEV_DEFAULTCODE_FILE) then begin
    AssignFile(txt, devDirs.Config + DEV_DEFAULTCODE_FILE);
    Rewrite(txt);  //write only
