...
var
  ...
  txt: TextFile;
begin
  if not FileExists(devDirs.Config + DEV_TOOLS_FILE) then begin
    AssignFile(txt, devDirs.Config + DEV_TOOLS_FILE);
    Rewrite(txt);  //write only
    value := '[Tools]' + #13#10 + 'Count=5' + #13#10 + #13#10 +
      '[Tool0]' + #13#10 +
      'Title=$FILEMANAGER$' + #13#10 +
      'Program=<WINDIR>\explorer.exe' + #13#10 +
      'WorkDir=<PROJECTPATH>' + #13#10 +
      'Params=/select, <PROJECTFILE>' + #13#10 + #13#10  +
      '[Tool1]' + #13#10  +
      'Title=$COMMANDPROMPT$' + #13#10  +
      'Program=<WINDIR>\system32\cmd.exe' + #13#10  +
      'WorkDir=<PROJECTPATH>' + #13#10  +
      'Params=' + #13#10  + #13#10  +

      '[Tool2]' + #13#10  +
      'Title=$TASKMANAGER$' + #13#10  +
      'Program=<WINDIR>\system32\taskmgr.exe' + #13#10  +
      'WorkDir=<DEFAULT>' + #13#10  +
      'Params=' + #13#10 + #13#10  +
      ...
      '[Tool4]' + #13#10 +
      'Title=$CALCULATOR$' + #13#10 +
      'Program=<WINDIR>\system32\calc.exe' + #13#10 +
      'WorkDir=<PROJECTPATH>' + #13#10 +
      'Params=' + #13#10;
    Writeln(txt, value);
    CloseFile(txt);
  ...
  with TINIFile.Create(devDirs.Config + DEV_TOOLS_FILE) do try
    ...
    for idx := 0 to pred(Count) do begin
      ...
      Item^.text := ReadString(section, 'Title', '');
...
begin
  ...
  with TINIFile.Create(devDirs.Config + DEV_TOOLS_FILE) do try
    ...
    for idx := 0 to pred(fList.Count) do begin
      ...
      section := 'Tool' + inttostr(idx);
      Item := fList[idx];
      WriteString(section, 'Title', Item.text);
