...
begin
  ...
  if Stage = cdPrePaint then begin  //before draw the node's main text
    Sender.Canvas.Font.Color := clWindowText; //clBlack;   //ABLI added
    Sender.Canvas.Font.Style := [fsBold];
    if bInherited then
      Sender.Canvas.Font.Color := clGrayText;  //clGray; //ABLI modi
    //the node's main text is drawn by TCustomTreeView
  end else if Stage = cdPostPaint then begin //draw the node's other text
    try
      st := Node.Data;
      ...
      if st^._Args <> '' then begin
        if bInherited then
          fControlCanvas.Font.Color := clGrayText //ABLI
        else
          fControlCanvas.Font.Color := clGrayText;
        fControlCanvas.TextOut(DrawPoint.X, DrawPoint.Y + 2, st^._Args); // center vertically
        Inc(DrawPoint.X, fControlCanvas.TextWidth(st^._Args) + 4); // add some right padding
      end;
