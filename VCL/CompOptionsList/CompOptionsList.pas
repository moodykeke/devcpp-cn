...
begin
  ...
  //if not ThemeServices.ThemesEnabled then   //XE2 Warning: Symbol 'ThemeServices' is deprecated: 'Use StyleServices'
  if not Enabled then  //TCustomStyleServices.Enabled
    Exit;
  ...
  if (ACol = 1) and (ARow >= FixedRows) and not (gdFocused in AState) and ItemProps[ARow - FixedRows].HasPickList then
    ...
    //Details := ThemeServices.GetElementDetails(tcDropDownButtonNormal);  //delphi 7
    Details := StyleServices.GetElementDetails(tcDropDownButtonNormal);  //delphi XE2
    //ThemeServices.DrawElement(Canvas.Handle, Details, ARect);    //delphi 7
    StyleServices.DrawElement(Canvas.Handle, Details, ARect);   //delphi XE2
  end;
end;
...
begin
  if Assigned(EditList) and enabled then //ThemeServices.ThemesEnabled then
