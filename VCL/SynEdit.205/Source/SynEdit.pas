  ...
  begin
    ...
    if bCanAppend then begin
      ...
      for i := 1 to TokenLen do
        TokenAccu.s[TokenAccu.Len + i] := Char(Token[i]); //abli
      ...
      for i := 1 to TokenLen do
        TokenAccu.s[i] := Char(Token[i]);   //abli
  ...
  begin
    if Filer.Ancestor <> nil then begin
      Result := not CollectionsEqual(Keystrokes,
        ...
        //TCustomSynEdit(Filer.Ancestor).Keystrokes, nil, nil);   //abli
    end else begin
      iDefKeys := TSynEditKeyStrokes.Create(nil);
      try
        iDefKeys.ResetDefaults;
        Result := not CollectionsEqual(Keystrokes, iDefKeys);
        //Result := not CollectionsEqual(Keystrokes, iDefKeys, nil, nil); //abli
      finally
        iDefKeys.Free;
      end;
    end;
...
begin
  ...
  P := PChar(Line);
  while CharInSet(P^, [#9, #32]) do begin
    if P^ = #9 then
      Inc(Result, TabWidth)
    else
      Inc(Result);
...
var
  ...
  begin
    ...
    for I := 1 to Length(CurLine) do begin
      if CurLine[i] = AnsiChar(character) then begin

        // Char must have proper highlighting (ignore stuff inside comments...)
        token2 := token;  //abli
        if (GetHighlighterAttriAtRowCol(BufferCoord(i, Line + 1), token2, attr) and (attr.Name =  //abli
  ...
  begin
    ...
    while Line < LinesToScan.Count do begin // index is valid for LinesToScan and fLines
      ...
      for Col := 1 to Length(CurLine) do begin
        ...
        if CurLine[col] = AnsiChar(fCodeFolding.FoldRegions[FoldIndex].Open) then begin //abli

          // Char must have proper highlighting (ignore stuff inside comments...)
          token2 := token;  //abli
          if (GetHighlighterAttriAtRowCol(BufferCoord(Col, Line + 1), token2, attr) and
        ...
        end else if CurLine[col] = AnsiChar(fCodeFolding.FoldRegions[FoldIndex].Close) then begin
