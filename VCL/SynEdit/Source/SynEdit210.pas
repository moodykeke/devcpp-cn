...
{$ENDIF}
    ...
    //procedure SetCaretXY(const Value: TBufferCoord); virtual; //move to public. by abli
    //procedure SetCaretXYEx(CallEnsureCursorPos: Boolean; Value: TBufferCoord); virtual;//move to public. by abli
...
{$ENDIF}
  ...
  public
    ...
    property GutterWidth: Integer read FGutterWidth;  //by abli
...
//++ DPI-Aware
          if Screen.PixelsPerInch >= 120 then
            fInternalImage.ChangeScale(Screen.PixelsPerInch, 120);  //96);//abli
//-- DPI-Aware
        end;
...
begin
  ...
  begin
   {$IFDEF SYN_COMPILER_18_UP}Strings.{$ENDIF}StrLCopy(PChar(Msg.Text), PChar(String(Text)), Msg.TextMax - 1);
    Msg.Result := {$IFDEF SYN_COMPILER_18_UP}Strings.{$ENDIF}StrLen(PChar(Msg.Text));
  end;
end;
    ...
    try
      ...
      begin
        ...
        try
          ...
          PWLength := MultiByteToWideChar(DefaultSystemCodePage, 0, PAnsiChar(PA), ImeCount,
            nil, 0);
          GetMem(PW, (PWLength + 1) * sizeof(WideChar));
          try
            MultiByteToWideChar(DefaultSystemCodePage, 0, PAnsiChar(PA), ImeCount,
              PW, PWLength);
            CommandProcessor(ecImeStr, #0, PW);
          finally
            FreeMem(PW);
...
begin
  ...
  try
    if HandleAllocated and IsWindowUnicode(Handle) then
      ...
      Text := UnicodeString(PChar(Msg.Text));
  except
    Msg.Result := 0;
    raise
  end
