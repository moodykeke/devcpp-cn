...
begin
  ...
  fOutput := StringReplace(fOutput, '->->display-begin' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->display-format' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, #13#10 + '->->display-number-end' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, #13#10 + '->->display-expression' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, #13#10 + '->->display-expression-end' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, #13#10 + '->->display-expression' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->display-end' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->frames-invalid' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->frame-end' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->pre-prompt' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '(gdb) ' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->prompt' + #13#10, '', [rfReplaceAll]);
  fOutput := StringReplace(fOutput, '->->post-prompt' + #13#10, '', [rfReplaceAll]);
  ...
  if (doupdatecpuwindow and not spawnedcpuform) and Assigned(CPUForm) then begin
    MainForm.Debugger.SendCommand('disas', '', true);
    MainForm.Debugger.SendCommand('info registers', '', true);
    MainForm.Debugger.SendCommand('backtrace', '', true);
  end;
end;
...
begin
  ...
  // eg. 'D:\обть\avrg.cpp:10:184:beg:0x401527'
  s := TrimLeft(GetRemainingLine);

  // remove offset, beg/middle/end, address. remain filename:line
