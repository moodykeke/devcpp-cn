...
uses
  ...
  SynEditCodeFolding, SynHighlighterCpp, SynExportHTML, SynEditTextBuffer, Math, StrUtils, SynEditTypes, SynEditHighlighter, DateUtils,
  CodeToolTip, CBUtils;
...
begin
  ...
  fText.OnMouseWheel := EditorMouseWheel;
  ...
  fText.OnScanForFoldRanges := ScanForFoldRanges;   // Scan for Folding//by abli
  //fText.Highlighter.Assign(devEditor.Syntax); //debug
...
begin
  ...
  fText.OnScanForFoldRanges := nil; //abli
  FreeAndNil(fText);
...
//      MainForm.GotoBookmarksPopItem.Items[i -1].Checked := true;
//      MainForm.GotoBookmarksItem.Items[i -1].Checked := true;
//    end else begin
//      MainForm.TogglebookmarksPopItem.Items[i - 1].Checked := false;
//      MainForm.TogglebookmarksItem.Items[i - 1].Checked := false;
//      MainForm.GotoBookmarksPopItem.Items[i -1].Checked := false;
//      MainForm.GotoBookmarksItem.Items[i -1].Checked := false;
//    end;
...
begin
  ...
  for I := 8 downto 0 do begin
    if BookmarkLines[I] < line then
      ...
      BookmarkLines[I+1] := BookmarkLines[I];
      if (BookmarkLines[I+1] <> 9999) then
        Text.SetBookmark(I+1, 1, BookmarkLines[I+1]);
    end;
  end;
  I := I + 1;
  BookmarkLines[I] := line;
  Text.SetBookmark(I, Text.CaretX, line);
end;

procedure TEditor.GotoPrevBookmark;
var
  I, line: integer;
begin
  line := Text.CaretY;
  ...
  if (I >= 0) and (BookmarkLines[I] < line) then
    Text.GotoBookMark(I)
  else
    Text.GotoLineAndCenter(0);
end;

procedure TEditor.GotoNextBookmark;
var
  I, line: integer;
begin
  line := Text.CaretY;
  ...
  if BookmarkLines[I] = 9999 then
    I := I - 1;
  if (I <=9) and (BookmarkLines[I] > line) then
    Text.GotoBookMark(I)
  else
    Text.GotoLineAndCenter(Text.Lines.Count-1);
end;
...
begin
  ...
  Value := StringReplace(Value, '$PROGRAM$', Lang[ID_ITEM_PROGRAM], [rfReplaceAll]);
  Value := StringReplace(Value, '$COPYRIGHT$', Lang[ID_ITEM_COPYRIGHT], [rfReplaceAll]);
  Value := StringReplace(Value, '$AUTHOR$', Lang[ID_ITEM_AUTHOR], [rfReplaceAll]);
  Value := StringReplace(Value, '$DATETIME$', Lang[ID_ITEM_DATEANDTIME], [rfReplaceAll]);
  Value := StringReplace(Value, '$DESCRIPTION$', Lang[ID_ITEM_DESCRIPTION], [rfReplaceAll]);
  Value := StringReplace(Value, '<DATETIME>', FormatDateTime(Lang[ID_ITEM_DATETIMEFORMAT], Now), [rfReplaceAll]);
  //Value := StringReplace(Value, '<DATETIME>', DateTimeToStr(Now), [rfReplaceAll]);
...
begin
  if devEditor.DefaultCode and FileExists(devDirs.Config + DEV_DEFAULTCODE_FILE) then begin
    ...
    try
      ...
      //InsertString(ParseMacros(tmp.Text), true);
      InsertString(tmp.Text, true);
    finally
      tmp.Free;
    end;
  end;
...
begin
  // Open up the closed folds around the focused line
  ...
  //fText.SetCaretXYCentered(True, BufferCoord(Col, Line)); //abli
  fText.SetCaretXYEx(True, BufferCoord(Col, Line)); //abli new.
  //SetCaretXYEx is protected in SynEdit v2.1.0. I change it as public.
end;
...
begin
  ...
  if fCompletionBox.Enabled then begin
    //if (Key in fText.IdentChars) then begin // Continue filtering   //deleted by abli
    if fText.IsIdentChar(Key) then begin // Continue filtering //abli new
      fText.SelText := Key;
...
var
  ...
  begin
    ...
    IndentCount := fText.LeftSpacesEx(fText.LineText, True);  //abli private?
  ...
  begin
//    if fSingleQuoteCompleteState <> scsInserted then begin
      InsertString('''', false);
//      fSingleQuoteCompleteState := scsInserted;
//    end else begin
//      fText.CaretXY := BufferCoord(fText.CaretX + 1, fText.CaretY); // skip over
...
//    end;
  ...
  begin
//    if fDoubleQuoteCompleteState <> scsInserted then begin
      InsertString('"', false);
//      fDoubleQuoteCompleteState := scsInserted;
//    end else begin
//      fText.CaretXY := BufferCoord(fText.CaretX + 1, fText.CaretY); // skip over
...
//    if (Attr = fText.Highlighter.StringAttribute) and CharInset(Key, ['"']) and devEditor.DoubleQuoteComplete then // strings
//      HandleDoubleQuoteCompletion;
//    if SameStr(Attr.Name, 'Character') and CharInset(Key, ['''']) and devEditor.SingleQuoteComplete  then
//      HandleSingleQuoteCompletion;
    if (Attr = fText.Highlighter.StringAttribute) or (Attr = fText.Highlighter.CommentAttribute)
        or SameStr(Attr.Name, 'Character') then
      Exit;
  end;
...
begin
  ...
  // Doing this heHandleSymbolCompletionre instead of in EditorKeyDown to be able to delete some key messages
...
procedure TEditor.ScanForFoldRanges(Sender: TObject;
...
var
  ...
  var
    ...
    Attr : TSynHighlighterAttributes;
  begin
    Result := fText.GetHighlighterAttriAtRowCol(BufferCoord(Col, Line + 1), Token, Attr) and
      (Attr = fText.Highlighter.CommentAttribute);
  end;
  ...
  if (p.Line >= 1) and (p.Line <= fText.Lines.Count) then begin
      ...
      while (WordEnd + 1 <= len) do begin
        if (Purpose = wpEvaluation) and (s[WordEnd + 1] = '[') then begin
          if not FindComplement(s, '[', ']', WordEnd, 1) then
            break;
        //end else if (s[WordEnd + 1] in fText.IdentChars) then //deleted by abli
        end else if fText.IsIdentChar(s[WordEnd + 1]) then // by abli
          Inc(WordEnd)
        else
          break;
      end;
    ...
    if Purpose in [wpCompletion, wpEvaluation, wpInformation] then begin
      while (WordBegin > 0) and (WordBegin <= len) do begin
        ...
        //end else if (s[WordBegin] in fText.IdentChars) then begin   //deleted by abli
        end else if fText.IsIdentChar(s[WordBegin]) then begin   //by abli
          Dec(WordBegin);
...
begin
  ...
  if (fTripleClickTime > fDblClickTime) and
    ...
    fText.StateFlags := fNewState;    //abli debug
...
var
  ...
  begin
    ...
    if (FileName <> '') and FileExists(FileName) then
      fText.Hint := FileName + ' - ' + Lang[ID_MSG_CLICKFORMOREINFO]
    else
      fText.Hint := '';
  end;
  ...
  begin
    ...
    MainForm.Debugger.SendCommand('print', s, true);
  end;
  ...
  begin
    ...
    if Assigned(st) then begin
      fText.Hint := MainForm.CppParser.PrettyPrintStatement(st) + ' - ' + ExtractFileName(st^._FileName) + ' (' +
        IntToStr(st^._Line) + ') - ' + Lang[ID_MSG_CLICKFORMOREINFO]; //'Ctrl+Click for more info';
...
procedure TEditor.EditorMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
   MainForm.FormMouseWheel(Sender, Shift, WheelDelta, MousePos, Handled);
end;
...
var
  ...
  begin
    ...
    if TransientType = ttAfter then begin // swap colors
      Canvas.Font.Color := fText.Highlighter.WhitespaceAttribute.Background;
...
begin
  ...
  if (fText.CaretX - 1 > 0) and (fText.CaretX - 1 <= LineLength)
      and CharInSet(fText.LineText[fText.CaretX - 1], AllChars) then
    HighlightCharPos := BufferCoord(fText.CaretX - 1, fText.CaretY)

    // Or after us?
  else if (fText.CaretX > 0) and (fText.CaretX <= LineLength)
           and CharInSet(fText.LineText[fText.CaretX], AllChars) then
  ...
  // Is the OpenChar before/after us highlighted as a symbol (not comment or something)?
  if not (fText.GetHighlighterAttriAtRowCol(HighlightCharPos, S, Attri)
          and (Attri = fText.Highlighter.SymbolAttribute)) then
    Exit;
  ...
  if fText.AllFoldRanges.FoldHidesLine(HighlightCharPos.Line) or
    fText.AllFoldRanges.FoldHidesLine(ComplementCharPos.Line) then
    Exit;
  ...
  //SetColors(HighlightCharPos);  // move down by abli
