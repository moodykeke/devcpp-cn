...
procedure TCppPreprocessor.SkipToEndOfPreprocessor;
  function LastLineChar(const Line: AnsiString): AnsiChar; //Char
  begin
    if Length(Line) > 0 then
...
var
  ...
  var
    ...
    begin
      ...
      try
        ExtractStrings([',', '(', ')'], [], PChar(FunctionDefine^.Args), ArgNames); //PAnsiChar
        ExtractStrings([',', '(', ')'], [], PChar(ArgValueString), ArgValues); //PAnsiChar // extract from Line string
