...
{$IFDEF SYN_CodeFolding}
    ...
    procedure SetClickCodeFolding(const Value: Boolean);
...
{$ENDIF}
    ...
    procedure InsertLine(const BB, BE: TBufferCoord; ChangeStr: PChar; AddToUndoList: Boolean);
...
{$IFDEF SYN_CodeFolding}
    ...
    property ClickCodeFolding: Boolean read fClickCodeFolding write SetClickCodeFolding;
...
{$IFDEF SYN_CodeFolding}
    ...
    property ClickCodeFolding;  //allow click gutter to folding
{$ENDIF}
...
begin
  ...
  if UseCodeFolding and ClickCodeFolding then begin
...
{$IFDEF SYN_CodeFolding}
  // Draw the folding squares and vertical lines.
  // (the following codes can be optimized more. )
  if UseCodeFolding then begin
    for cRow := aFirstRow to aLastRow do begin
      ...
      // Any fold ranges around on this line?
      // Need to draw three part: upper vertical line, middle part, down vertical line
      // The middle part may be a beginning box or a simple vertical line.
      if AllFoldRanges.FoldAroundLine(vLine, Index) then begin
        x := rcFold.Left + ((rcFold.Right - rcFold.Left) div 2);
        // Draw the upper part of a line
        Canvas.MoveTo(x, rcFold.Top);
        Canvas.LineTo(x, (cRow - fTopLine) * LineHeight - 2);
        // Draw the bottom part of a line
        Canvas.MoveTo(x, rcFold.Bottom);
        Canvas.LineTo(x, (cRow - fTopLine + 1) * LineHeight + 2);
      end;
      ...
      else begin   // not FoldStartAtLine(vLine, Index)
...
procedure TCustomSynEdit.SetClickCodeFolding(const Value: Boolean);
begin
  fClickCodeFolding := Value and ((Assigned(fHighlighter) and
      (fHighlighter is TSynCustomCodeFoldingHighlighter))
        or Assigned(fOnScanForFoldRanges)) and fUseCodeFolding;
end;
...
begin
  ...
  if (not UseCodeFolding) or (not ClickCodeFolding) then
    Exit;
...
// too, so they could be moved depending on whether they are after the caret...
          ...
          if Len > 0 then begin
            if Len >= CaretX then begin
              ...
              Lines.Insert(CaretY, '');
              Caret := CaretXY;
//the following lines are replaced by KV, then restored by abli.
                if Command = ecLineBreak then begin
                  ...
                  if (eoAddIndent in Options) and GetHighlighterAttriAtRowCol(BufferCoord(Length(Temp), CaretY),
                    Temp, Attr) then begin // only add indent to source files, not to comment
                    if (Attr <> Highlighter.CommentAttribute) then begin // and outside of comments
                      if Temp[Length(Temp)] in ['{', ':'] then begin // add more indent for these too
                        if not (eoTabsToSpaces in Options) then begin
                          Lines[CaretY] := Lines[CaretY] + #9;  //TSynTabChar;
                          Inc(SpaceCount2, 1);
                        end else begin
...
{              FUndoList.AddChange(crLineBreak, Caret, Caret, '', smNormal);   //KV
      ...
      // #127 is Ctrl + Backspace, #32 is space
        if not ReadOnly and (AChar >= #32) and (AChar <> #127) then begin
          ...
          //don't input duplicate ')' or ']' or '}' in current position
//          if (CharInSet(AChar, [')', ']','}'])) and samestr(Achar, LineText[CaretX]) then begin
//            AChar := #0;
//            CaretX := CaretX + 1;
//          end;
            ...
            try
              ...
              if FInserting then begin
                if not WordWrap and not (eoAutoSizeMaxScrollWidth in Options)
                   and (CaretX > MaxScrollWidth) then begin
                  Exit;
                end;
...
{$IFDEF SYN_CodeFolding}
  ...
  if (ptCursor.X < fGutterWidth) then begin
    ...
    begin
      ...
      if PtInRect(Rect, ptCursor) and ClickCodeFolding then
