...
{$endif}
  ...
  TStringDataType = (stUnknown, stString, stUnicodeString);
...
begin
  Result := THandle(__open(PChar(UTF8String(FileName)), O_RDWR or O_CREAT or O_TRUNC or O_EXCL, FileAccessRights));
end;
...
{$endif}
  ...
  {Get the module name}
{$ifndef POSIX}
  Result := GetModuleFileNameA(LModuleHandle, PAnsiChar(ABuffer), 512);
{$else}
...
begin
  ...
  begin
    ...
    MessageBoxA(0, PAnsiChar(AText), PAnsiChar(ACaption),
  ...
  begin
    ...
    begin
      case DetectStringData(Pointer(PByte(APointer) + SizeOf(TFullDebugBlockHeader)), APointer.UserSize) of
        ...
        stString: LMsgPtr := AppendStringToBuffer(StringBlockMessage, LMsgPtr, Length(StringBlockMessage));
        ...
        try
          ...
          begin
            ...
            case NativeInt(LPNode.ClassPtr) of
              ...
              1:
              begin
                LPMsg := AppendStringToBuffer(StringBlockMessage, LPMsg, Length(StringBlockMessage));
              end;
              {UnicodeString}
              2:
              begin
