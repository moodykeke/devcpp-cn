...
type
  ...
  private
    fTemplates: TList;
    fMsgTrans: TStringList; //project information translation
...
begin
  ...
  fMsgTrans := TStringList.Create;
//  procedure TdevMultiLangSupport.LoadMsg(const Filename: String; var strlist: TStringList);
// Load file from Templates directory

  TransFile := ChangeFileExt(devData.Language, '.translate');
  TransFile := ValidateFile(TransFile, devDirs.Templates);
  if FileExists (TransFile) then begin
    fMsgTrans.LoadFromFile(TransFile);
    fMsgTrans.Delete(0);
  end;
...
begin
  ...
  for i := 0 to (fMsgTrans.count div 2 - 1) do begin
    Template.Category := StringReplace(Template.Category, fMsgTrans[i*2] , fMsgTrans[i*2+1], []);
    Template.Name := StringReplace(Template.Name, fMsgTrans[i*2] , fMsgTrans[i*2+1], []);
    Template.Description := StringReplace(Template.Description, fMsgTrans[i*2] , fMsgTrans[i*2+1], []);
  end;
