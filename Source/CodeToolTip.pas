...
type
  ...
  protected
    ...
    function DummyPaint(RealDo: boolean): integer; //Paint and obtain tooltip width
...
//(It is not easy to merge. because override Paint is trigged by WinProc.)
// Paints the tooltip to FBmp and copies it to the tooltip client surface.
// if RealDo = TRUE, then really draw.
// if RealDo = FALSE, then only calculate how much width we need to paint.
function TCodeToolTip.DummyPaint(RealDo: boolean): integer;
var
...
begin
  Result := 4; //Painting Width. initiate with some margin on the left and right sides
  ...
  if FToolTips.Count > 1 then begin
    // paint the UP button
    if RealDo then FUpButton.Paint(FBmp.Canvas);
    ...
    if Realdo then FBmp.Canvas.TextOut(Result, 1, S);
    Inc(Result, FBmp.Canvas.TextWidth(S) + 3);

    // paint the DOWN button
    FDownButton.Left := Result;
    if RealDo then FDownButton.Paint(FBmp.Canvas);
    Inc(Result, 3 + FDownButton.Width + FUpButton.Left);
  end;

  //the exact painting need to check every character and set their style.
  //However, the method "FLookupEditor.CaretX" may cause error: Control has no parent window .
  // I am unable to deal with it. so I made a simple solution. //by abli, 2020-04-16

  if Realdo then begin
    FBmp.Canvas.TextOut(Result, 1, Caption);   //by abli
    Canvas.CopyRect(ClientRect, FBmp.Canvas, ClientRect);  //by abli
  end;
  Inc(Result, FBmp.Canvas.TextWidth(Caption) + 4);  //Text width of "Caption" and right margin. //abli

  exit; // the above is a simple solution. Explanation as above. //by abli
  ...
  for CurPos := 1 to Length(Caption) do begin
    ...
    FLookupEditor.CaretX := CurPos;   //the second time runnning this line will cause error. //abli
    ...
    with FBmp.Canvas do begin
      ...
      if RealDo then TextOut(Result, 1, Caption[CurPos]);
      Inc(Result, TextWidth(Caption[CurPos]));
    end;
  end;
  Inc (Result, 4);  //Right margin
end;
...
begin
  ...
  FBmp.Canvas.Font.Color := clBlack;  //by abli
  FBmp.Canvas.Font.Style := [fsBold]; //by abli
  FBmp.Canvas.TextOut(WidthParam, 1, Caption);   //by abli
  Canvas.CopyRect(ClientRect, FBmp.Canvas, ClientRect);  //by abli
  exit; // the above is a simple solution. Explanation in DummyPaint. //by abli
...
begin
  ...
  Pt.X := FEditor.ClientToScreen(FEditor.RowColumnToPixels(FEditor.BufferToDisplayPos(FEditor.CharIndexToRowCol(FTokenPos)))).X;
  Pt.Y := FEditor.ClientToScreen(FEditor.RowColumnToPixels(FEditor.BufferToDisplayPos(FEditor.CharIndexToRowCol(FFunctionEnd)))).Y;

  ActivateHint(Rect(Pt.X,
    Pt.Y + FEditor.LineHeight + 2,
    Pt.X + DummyPaint(FALSE),
